...
async function sendBuyerOrderEmail(to, books) {
  const booksListHtml = books.map(book => `<li><strong>${book.title}</strong> - ${book.price} ${book.priceCurrency}</li>`).join("");
...
async function sendSellerOrderEmail(to, books) {
...
module.exports = { sendBuyerOrderEmail, sendSellerOrderEmail };